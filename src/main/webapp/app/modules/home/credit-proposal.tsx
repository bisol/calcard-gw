...
import { Button, Col, Row, Table, Form, Input, InputGroup, InputGroupAddon } from 'reactstrap';
...
export interface ICreditProposalState extends IPaginationBaseState {
  cpfQuery: string;
}
...
export class CreditProposal extends React.Component<ICreditProposalProps, ICreditProposalState> {
  ...
  getEntities = (e: any) => {
    ...
    this.props.getEntities(this.state.cpfQuery, activePage - 1, itemsPerPage, `${sort},${order}`);
  };

  setQuery = (e: any) => {
    // console.log('change', args);
    this.setState({
      cpfQuery: e.target.value
    });
  };
  ...
  render() {
    ...
    return (
      <div>
        ...
        <Row>
          <Form model="cpfQuery" onSubmit={this.getEntities}>
            <InputGroup>
              <InputMask
                id="cpf-query-id"
                type="tel"
              ...
              <thead>
                <tr>
                  ...
                  <th className="hand" onClick={this.sort('gender')}>
                    <Translate contentKey="calcardApp.creditProposal.gender">Gender</Translate> <FontAwesomeIcon icon="sort" />
                  </th>
